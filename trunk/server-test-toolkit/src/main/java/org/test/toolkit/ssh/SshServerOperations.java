...
import org.test.toolkit.ssh.exception.CommandExecuteException;
import org.test.toolkit.ssh.exception.ServerConnectionException;
import org.test.toolkit.ssh.exception.ServerTimeoutException;
...
public class SshServerOperations extends AbstractServerOperations {
	...
	private SshServerOperations(SshUser atLeastOneSshUser, SshUser... otherSshUsers) {
		...
		List<SshUser> allSshUsers = CollectionUtils.getList(atLeastOneSshUser, otherSshUsers);
 		for (SshUser sshUser : allSshUsers) {
			String ip = sshUser.getIp();
	...
	private Session getSshSession(SshUser sshUser) {
		...
		JSch jSch = new JSch();
		try {
			session = jSch.getSession(sshUser.getUserName(), sshUser.getIp(), sshUser.getPort());
