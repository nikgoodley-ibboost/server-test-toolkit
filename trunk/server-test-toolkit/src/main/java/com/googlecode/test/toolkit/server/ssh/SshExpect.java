 ...
 * @version 1.1
 */
public class SshExpect {
	static final Logger log = Logger.getLogger(SshExpect.class);
	...
	public SshExpect(InputStream input, OutputStream output) {
		try {
	...
	/**
	 * @return the spawned process, if this {@link SshExpect} object is created by
	 *         spawning a process
	 */
	 ...
	 *         spawned process
	 */
	public static SshExpect spawn(String command) {
		...
		SshExpect retv = new SshExpect(p.getInputStream(), p.getOutputStream());
		retv.process = p;
		return retv;
	}
	...
	public static void forwardInputStreamTo(PrintStream duplicatedTo) {
		SshExpect.duplicatedTo = duplicatedTo;
	}
